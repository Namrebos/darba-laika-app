...
create unique index if not exists work_logs_created_at_idx
  on public.work_logs (created_at);

create unique index if not exists task_logs_offline_id_idx
  on public.task_logs (offline_id)
  where offline_id is not null;

create unique index if not exists task_images_task_url_idx
  on public.task_images (task_log_id, url);

create or replace function public.sync_offline_workday(
  client_id uuid,
...
begin
  ...
  insert into public.work_logs (created_at, user_id, project, description, start_time, end_time)
  values (client_id, auth.uid(), 'Darba diena', '', started_at, ended_at)
  on conflict (created_at) do update
    set start_time = excluded.start_time,
        end_time = excluded.end_time
    where public.work_logs.user_id = auth.uid()
  returning id into result_id;

  if result_id is null then raise exception 'Workday belongs to another user'; end if;
  return result_id;
end;
$$;

revoke all on function public.sync_offline_workday(uuid, timestamptz, timestamptz) from public, anon;
grant execute on function public.sync_offline_workday(uuid, timestamptz, timestamptz) to authenticated;
...
begin
  ...
  if existing_task_id is not null then
    ...
    set offline_id = coalesce(offline_id, client_id),
        title = task_title,
        note = task_note,
