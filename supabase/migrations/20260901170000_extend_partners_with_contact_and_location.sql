...
set display_name = case
  when partner_type = 'company' then company_name
...
alter table public.partners
  add constraint partners_display_name_not_blank
    check (nullif(btrim(display_name), '') is not null),
  add constraint partners_location_pair_check
    check (
...
create index partners_display_name_idx
  on public.partners (lower(display_name));
