...
set
  pickup_contact_name = coalesce(pickup_contact_name, sender_company_name, trim(concat_ws(' ', sender_first_name, sender_last_name))),
  pickup_contact_phone = coalesce(pickup_contact_phone, sender_phone),
  dropoff_contact_name = coalesce(dropoff_contact_name, recipient_company_name, trim(concat_ws(' ', recipient_first_name, recipient_last_name))),
  dropoff_contact_phone = coalesce(dropoff_contact_phone, recipient_phone)
where pickup_contact_name is null
   or pickup_contact_phone is null
   or dropoff_contact_name is null
   or dropoff_contact_phone is null;
...
begin
  ...
  insert into public.transport_requests (
    link_id, created_by,
    sender_type, sender_first_name, sender_last_name, sender_company_name, sender_registration_number, sender_phone,
    recipient_type, recipient_first_name, recipient_last_name, recipient_company_name, recipient_registration_number, recipient_phone,
    ...
    payload->>'sender_type', nullif(trim(payload->>'sender_first_name'), ''), nullif(trim(payload->>'sender_last_name'), ''), nullif(trim(payload->>'sender_company_name'), ''), nullif(trim(payload->>'sender_registration_number'), ''), trim(payload->>'sender_phone'),
    'private', nullif(trim(payload->>'dropoff_contact_name'), ''), null, null, null, trim(payload->>'dropoff_contact_phone'),
    nullif(trim(payload->>'pickup_contact_name'), ''), trim(payload->>'pickup_contact_phone'), nullif(trim(payload->>'dropoff_contact_name'), ''), trim(payload->>'dropoff_contact_phone'),
    trim(payload->>'pickup_address'), (payload->>'pickup_lat')::double precision, (payload->>'pickup_lng')::double precision, (payload->>'pickup_date')::date, nullif(trim(payload->>'pickup_time'), '')::time, coalesce(trim(payload->>'pickup_notes'), ''),
    trim(payload->>'dropoff_address'), (payload->>'dropoff_lat')::double precision, (payload->>'dropoff_lng')::double precision, (payload->>'dropoff_date')::date, nullif(trim(payload->>'dropoff_time'), '')::time, coalesce(trim(payload->>'dropoff_notes'), ''),
