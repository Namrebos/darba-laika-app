    ...
    <div className="border-t border-zinc-200 pt-4 dark:border-zinc-700">
      ...
      {form.partner_type === "company" ? <div className="grid gap-3 sm:grid-cols-2">
        <div className="relative" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) { setFocused(false); setSuggestions([]); } }}>
          ...
          {suggestions.length > 0 && <div className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border bg-white p-1 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">{suggestions.map((company) => <button key={`${company.registrationNumber}-${company.name}`} type="button" onPointerDown={(event) => { event.preventDefault(); chooseCompany(company); }} onClick={() => chooseCompany(company)} className="block w-full rounded-lg px-3 py-2 text-left hover:bg-blue-50 dark:hover:bg-zinc-800"><strong className="block">{company.name}</strong><span className="text-xs text-zinc-500">Reģ. Nr. {company.registrationNumber}</span></button>)}</div>}
        </div>
