...
export default function UsersPage() {
  ...
  const [permissionDrafts, setPermissionDrafts] = useState<Record<string, SectionPermissions>>({});
  const [summaryAccessDrafts, setSummaryAccessDrafts] = useState<Record<string, string[]>>({});
  ...
  useEffect(() => {
    async function load() {
      ...
      setProfiles(rows);
      setPermissionDrafts(Object.fromEntries(rows.map((profile) => [
        profile.id,
        Object.fromEntries(sectionOptions.map(({ key }) => [key, profile[key] === true])) as SectionPermissions,
      ])));
      ...
      setSummaryAccessDrafts(Object.fromEntries(
        Object.entries(accessMap).map(([viewerId, ownerIds]) => [viewerId, [...ownerIds]]),
      ));
  ...
  async function saveUserAccess(profile: AccessProfile) {
    ...
    const allowedOwnerIds = (summaryAccessDrafts[profile.id] || []).filter((ownerId) => ownerId !== profile.id);
    ...
    if (permissionsError) {
      setSavingUserId("");
      setMessage("Lietotāja piekļuves neizdevās saglabāt.");
      return;
    }
    ...
    const { error: insertError } = !deleteError && allowedOwnerIds.length > 0
      ? await supabase.from("summary_access").insert(
          allowedOwnerIds.map((ownerId) => ({ viewer_id: profile.id, owner_id: ownerId })),
        )
      : { error: null };

    setSavingUserId("");
    if (deleteError || insertError) {
      setMessage("Sadaļu piekļuves saglabātas, bet kopsavilkumu piekļuves neizdevās saglabāt.");
      return;
    }

    setProfiles((current) => current.map((item) =>
      item.id === profile.id ? { ...item, ...permissions } : item,
    ));
    setSummaryAccess((current) => ({ ...current, [profile.id]: allowedOwnerIds }));
    setMessage(`Lietotāja ${profile.display_name || profile.email || ""} piekļuves saglabātas.`);
  }
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    {sectionOptions.map(({ key, label }) => (
                      <label key={key} className="flex items-center gap-2 text-sm">
                        <input
                          type="checkbox"
                          checked={profile.role === "admin" || permissionDrafts[profile.id]?.[key] === true}
                          disabled={profile.role === "admin"}
                          onChange={(event) =>
                            setPermissionDrafts((current) => ({
                              ...current,
                              [profile.id]: {
                                ...(current[profile.id] || emptyPermissions),
                                [key]: event.target.checked,
                              },
                            }))
                          }
                  ...
                  <>
                          ...
                          return (
                            <label key={owner.id} className="flex items-center gap-2 text-sm">
                              <input
                                ...
                                checked={
                                  isOwnProfile ||
                                  (summaryAccessDrafts[profile.id] || []).includes(owner.id)
                                }
                                disabled={isOwnProfile}
                                onChange={(event) =>
                                  setSummaryAccessDrafts((current) => ({
                                    ...current,
                                    [profile.id]: event.target.checked
                                      ? Array.from(new Set([...(current[profile.id] || []), owner.id]))
                                      : (current[profile.id] || []).filter((id) => id !== owner.id),
                                  }))
                                }
                              />
                              <span>
                    ...
                    <div className="border-t border-zinc-200 pt-3 dark:border-zinc-700">
                      <div className="flex flex-wrap justify-between gap-2">
                        <button
                          type="button"
                          disabled={deletingUserId === profile.id || savingUserId === profile.id}
                        ...
                        <button
                          type="button"
                          disabled={savingUserId === profile.id || deletingUserId === profile.id}
                          onClick={() => void saveUserAccess(profile)}
                          className="rounded bg-green-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
                        >
                          {savingUserId === profile.id ? "Saglabā..." : "Saglabāt"}
                        </button>
                      </div>
                    </div>
                  </>
